...
def build_cursor(results, key, limit=100, is_desc=False, cursor=None, hits=None, max_hits=None):
    ...
    if is_prev and num_results:
        prev_value = int(key(results[first_prev_index], for_prev=True))
        ...
        if prev_value == value:
            ...
            # Always move past the first item, this is the prev value item and will
        ...
        # If we're paging back we need to calculate the key from the first
        # result with for_prev=True to ensure rounding of the key is correct.
        # See sentry.api.paginator.BasePaginator.get_item_key
